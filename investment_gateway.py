...
async def leaders():
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        ...
        symbols = [
            ...
            and x.get("type") == "STOCK"
            and x.get("board") in ["HSX", "HNX", "UPCOM"]
            and x.get("symbol")
        ]
        ...
        for x in prices:
            listing = x.get("listingInfo", {})
            match = x.get("matchPrice", {})

            try:
                ref = float(listing.get("refPrice"))
                price = float(match.get("matchPrice"))
